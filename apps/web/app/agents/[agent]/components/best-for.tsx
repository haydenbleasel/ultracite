...
export const BestFor = ({ agent }: BestForProps) => {
  // `useCases` is a data property, not a hook — react-compiler's use-prefix
  // heuristic misreads it, so disable the rule for this one mapping.
  // eslint-disable-next-line react/react-compiler
  const cards = agent.content.useCases.map((item, index) => (
