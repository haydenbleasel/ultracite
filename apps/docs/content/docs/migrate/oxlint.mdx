...
description: How to migrate your project from OxLint to Ultracite.
---
...
- **Combined Tool**: Ultracite provides both linting and formatting in a single tool
...
- An OxLint configuration file (e.g., `oxlintrc.json`, `.oxlintrc.json`)
...
When prompted, choose to remove the existing OxLint configuration. This will:

- Remove OxLint and any related dependencies
- Remove any OxLint configuration files
- Remove any OxLint related commands from your `.vscode/settings.json` file
...
- Manually removing OxLint and its configuration files
