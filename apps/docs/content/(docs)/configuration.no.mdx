---
title: Konfigurasjon
description: Ultracites standard lint-konfigurasjon og hvordan du endrer den.
---
...
You can find the full list of available framework configurations in the [Forhåndsinnstillinger](/preset/core) section.
...
- **TypeScript-strenghet**: Ultracite enables TypeScript's strict checks and lint rules requiring robust typing. For instance, it discourages use of `any`, requires handling of `null/undefined`, and prefers explicit types in certain situations to avoid ambiguity.
...
- **Tilgjengelighet**: Rules equivalent to accessibility standards are on by default (ARIA attributes, semantic HTML, keyboard navigation, etc.).
...
For a comprehensive reference of all linting rules configured in Ultracite's base configuration, see the [Kjernekonfigurasjon](/preset/core) documentation.

## Overstyre konfigurasjonen
