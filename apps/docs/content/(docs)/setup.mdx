...
```package-install
npm install -D -E ultracite @biomejs/biome
```
...
This extends Ultracite’s preset configuration. The JSON schema reference helps with IntelliSense and validation in editors.

### 3. Create a `.vscode/settings.json` file

Ultracite is designed to work best with VS Code and VS Code-based editors (like Cursor and Windsurf) with on-save formatting enabled. Create or update your workspace `.vscode/settings.json` with the following configuration:
