...
If you want to manually set things up, create the relevant file in your project directory (for example `.cursor/hooks.json` for Cursor or `.rules` for Zed).
...
> The hook ensures Cursor runs `npx ultracite fix` after each edit. If you already have hooks configured, just append the Ultracite command to `afterFileEdit`.
