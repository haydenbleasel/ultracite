...
You can pass any Biome CLI flags to customize the behavior:
...
npx ultracite check --max-diagnostics 50

# Check specific files
npx ultracite check src/index.ts src/utils.ts
```
...
# Enable verbose output
npx ultracite fix --verbose

# Fix specific files
npx ultracite fix src/index.ts src/utils.ts
```

For a complete list of available flags, see the [Biome CLI documentation](https://biomejs.dev/reference/cli/).
