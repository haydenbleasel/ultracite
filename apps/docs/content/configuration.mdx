...
Here is a comprehensive reference of all linting rules configured in Ultracite's `biome.jsonc`, organized by category.
...
| Rule | Setting | Description |
|------|---------|-------------|
| `noChildrenProp` | `error` | Prevent passing of children as props. |
...
| `noNestedComponentDefinitions` | `error` | Disallows defining React components inside other components. |
...
| `noReactPropAssignments` | `error` | Disallow assigning to React component props. |
| `noRenderReturnValue` | `error` | Prevent the usage of the return value of `React.render`. |
...
| `noSolidDestructuredProps` | `off` | Disallow destructuring props inside JSX components in Solid projects. |
...
| `noVoidElementsWithChildren` | `error` | Prevents void elements (AKA self-closing elements) from having children. |
...
| `useJsonImportAttributes` | `error` | Enforces the use of with `{ type: "json" }` for JSON module imports. |
| `useJsxKeyInIterable` | `error` | Disallow missing key props in iterators/collection literals. |
...
| Rule | Setting | Description |
|------|---------|-------------|
| `noDeprecatedImports` | `error` | Restrict imports of deprecated exports. |
...
| `noMisusedPromises` | `error` | Disallow Promises to be used in places where they are almost certainly a mistake. |
| `noNextAsyncClientComponent` | `error` | Prevent client components from being async functions. |
| `noNonNullAssertedOptionalChain` | `error` | Disallow non-null assertions after optional chaining expressions. |
| `noQwikUseVisibleTask` | `error` | Disallow `useVisibleTask$()` functions in Qwik components. |
| `noReactForwardRef` | `error` | Replaces usages of `forwardRef` with passing ref as a prop (React 19+). |
...
| `noVueDataObjectDeclaration` | `error` | Enforce that Vue component data options are declared as functions. |
| `noVueDuplicateKeys` | `error` | Disallow duplicate keys in Vue component data, methods, computed properties, and other options. |
| `noVueReservedKeys` | `error` | Disallow reserved keys in Vue component data and computed properties. |
| `noVueReservedProps` | `error` | Disallow reserved names to be used as props. |
...
| `useExhaustiveSwitchCases` | `error` | Require switch-case statements to be exhaustive. |
...
| `useQwikClasslist` | `off` | Prefer using the class prop as a classlist over the classnames helper. |
| `useQwikMethodUsage` | `off` | Disallow `use*` hooks outside of `component$` or other `use*` hooks in Qwik applications. |
| `useQwikValidLexicalScope` | `off` | Disallow unserializable expressions in Qwik dollar (`$`) scopes. |
| `useReactFunctionComponents` | `error` | Enforce that components are defined as functions and never as classes. |
| `useSortedClasses` | `error` (safe fix override) | Enforce the sorting of CSS utility classes. |
| `useVueMultiWordComponentNames` | `off` | Enforce Vue component names to be multi-word. |
...
| `noImgElement` | `error` | Prevent usage of `<img>` element in a Next.js project. |
...
| `useSolidForComponent` | `off` | Enforce using Solid's `<For />` component for mapping an array to JSX elements. |
...
| `noHeadElement` | `error` | Prevent usage of `<head>` element in a Next.js project. |
...
| `noArrayIndexKey` | `error` | Discourage the usage of Array index in keys. |
...
| `noDocumentImportInPage` | `error` | Prevents importing `next/document` outside of `pages/_document.jsx` in Next.js projects. |
...
| `noDuplicateJsxProps` | `error` | Prevents JSX properties to be assigned multiple times. |
...
| `noHeadImportInDocument` | `error` | Prevent using the `next/head` module in `pages/_document.js` on Next.js projects. |
...
| `noReactSpecificProps` | `off` | Prevents React-specific JSX properties from being used. |
...
| `noSuspiciousSemicolonInJsx` | `error` | It detects possible `wrong` semicolons inside JSX elements. |
