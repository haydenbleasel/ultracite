...
2. **Only the Edited File**: `--hook` reads the agent's hook payload from stdin and fixes just the file it names, so an edit costs one file instead of the whole project. Claude Code, CodeBuddy, Cursor and Windsurf send the path. A file outside the project, or one that no longer exists, is skipped. When no path can be read from the payload, the hook fixes the whole project as before.
