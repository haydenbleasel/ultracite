...
This is **off by default** because it adds dependencies and runs a slower JavaScript lint pass instead of the native Rust one. Interactive `ultracite init` lets you choose individual JS plugins when you choose Oxlint; for non-interactive setup, pass package names to `--js-plugins`.
