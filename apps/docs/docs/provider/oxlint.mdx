...
The core preset runs entirely on Oxlint's native, Rust-powered rules for maximum speed. If you want to close the remaining gap with the ESLint preset, Ultracite ships an opt-in `js-plugins` preset that runs [`eslint-plugin-github`](https://github.com/github/eslint-plugin-github) and [`eslint-plugin-sonarjs`](https://github.com/SonarSource/eslint-plugin-sonarjs) through Oxlint's JS plugin support.

This is **off by default** because it adds two dependencies and runs a slower JavaScript lint pass instead of the native Rust one. Opt in only if you want the extra rules and accept the trade-off.
...
For extra ESLint parity, the optional **js-plugins** preset adds **github** (`eslint-plugin-github`) and **sonarjs** (`eslint-plugin-sonarjs`) via Oxlint's JS plugin support. See [ESLint Parity](#eslint-parity-optional) — it is not enabled by default.
