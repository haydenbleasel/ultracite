...
export default defineConfig({
  extends: [core, github, sonarjs],
  ignorePatterns: core.ignorePatterns,
});
```

The `github` and `sonarjs` presets run `eslint-plugin-github` and `eslint-plugin-sonarjs` through Oxlint's JS plugin support to close the gap with the ESLint preset. They're shipped as separate presets so you can drop either one to opt out.
...
In addition, the default generated config bundles **github** (`eslint-plugin-github`) and **sonarjs** (`eslint-plugin-sonarjs`) as separate presets via Oxlint's JS plugin support.
