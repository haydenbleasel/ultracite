...
Ultracite can be installed with all major package managers. You'll need Node.js 20+ and a project with a `package.json` file. If you're new to Ultracite, the [introduction](/docs) covers what it is and which toolchains it supports.
...
| --- | --- | --- |
| `--linter` | The linting toolchain to use. Defaults to `oxlint` when prompts are skipped. | `oxlint`, `biome`, `eslint` |
| `--pm` | The package manager to use. | `pnpm`, `bun`, `yarn`, `npm`, `deno`, `nub`, `aube` |
...
| `--js-plugins` | Optional Oxlint JS plugins to enable. | Space-separated: `@shadcn/lint`, `anti-slop`, `eslint-plugin-github`, `eslint-plugin-sonarjs`, `oxlint-plugin-react-doctor` |
...
- Use sensible defaults for all options (Oxlint + Oxfmt by default)
