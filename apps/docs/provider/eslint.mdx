...
- **TypeScript-first** — Deep TypeScript integration via @typescript-eslint with strict type checking rules
- **Maximum coverage** — Hundreds of rules covering accessibility, security, performance, and best practices

<Callout>
  The `--type-aware` CLI flag applies to Biome and Oxlint only. For ESLint, TypeScript support is provided natively by the `@typescript-eslint` plugin included in the core config.
</Callout>
