...
describe("check", () => {
  ...
  test("passes through unknown options to biome", () => {
    ...
    check([], ["--diagnostic-level=error", "--error-on-warnings"]);
  ...
  test("throws LinterExitError when biome check finds errors", () => {
    ...
    expect(() => check()).toThrow("Biome exited with code 1");
  });
  ...
  test("exits when spawn returns error", () => {
    ...
    expect(() => check()).toThrow("Failed to run Biome: spawn failed");
  });
  ...
  test("eslint check throws on eslint spawn error", () => {
    ...
    expect(() => check()).toThrow("Failed to run ESLint: eslint spawn failed");
  });
  ...
  test("oxlint check throws on spawn error", () => {
    ...
    expect(() => check()).toThrow("Failed to run oxfmt: oxfmt spawn failed");
  });
  ...
  test("fails before running biome when its extends cannot be resolved", () => {
    ...
    try {
      expect(() => check()).toThrow(/could not be resolved/u);
