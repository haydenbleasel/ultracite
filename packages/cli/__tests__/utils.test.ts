...
import { exists, isMonorepo, parseFilePaths, updatePackageJson } from '../src/utils';
...
describe('updatePackageJson', () => {
  test('updates devDependencies', async () => {
    ...
    mock.module('node:fs/promises', () => ({
      access: mock(() => Promise.resolve()),
      readFile: mock(() => Promise.resolve('{"name": "test", "devDependencies": {"old": "1.0.0"}}')),
  ...
  test('updates dependencies', async () => {
    ...
    mock.module('node:fs/promises', () => ({
      access: mock(() => Promise.resolve()),
      readFile: mock(() => Promise.resolve('{"name": "test", "dependencies": {"old": "1.0.0"}}')),
      writeFile: mockWriteFile,
    }));
  ...
  test('updates scripts', async () => {
    ...
    mock.module('node:fs/promises', () => ({
      access: mock(() => Promise.resolve()),
      readFile: mock(() => Promise.resolve('{"name": "test", "scripts": {"test": "echo test"}}')),
      writeFile: mockWriteFile,
    }));
...
describe('parseFilePaths', () => {
  ...
  test('wraps files with special characters in quotes', () => {
    ...
    const result = parseFilePaths(files);
    expect(result).toEqual(["'src/file(1).ts' ", "'test[2].js' ", "'file&test.ts' "]);
  });

  test('handles mixed file paths', () => {
    const files = ['normal.ts', 'with space.js', "user's.ts"];
    const result = parseFilePaths(files);
    expect(result).toEqual(['normal.ts', "'with space.js' ", "'user'\\''s.ts' "]);
  });
});
