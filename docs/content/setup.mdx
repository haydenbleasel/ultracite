...
```package-install
npm install -D -E ultracite @biomejs/biome@2.1.1
```
...
This extends Ultracite’s preset configuration. The JSON schema reference helps with IntelliSense and validation in editors, but you can use the latest schema URL corresponding to the Biome version you installed (e.g., adjust `2.1.1` to the current Biome version if different).
