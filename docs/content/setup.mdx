...
```package-install
npm install --save-exact ultracite @biomejs/biome@2.0.0
```
...
This extends Ultracite’s preset configuration. The JSON schema reference helps with IntelliSense and validation in editors, but you can use the latest schema URL corresponding to the Biome version you installed (e.g., adjust `2.0.0` to the current Biome version if different).
