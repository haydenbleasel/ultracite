...
## Q: Can I use Ultracite without VS Code / Cursor?

Yes. While VS Code integration is a highlight (because it automates the fixes on save), Ultracite can be used purely via CLI or with [other editor setups](https://biomejs.dev/guides/editors/first-party-extensions/).

You might run Ultracite manually or on git hooks as described. Some users use Vim/Neovim with Biome’s LSP for on-the-fly linting. Others might just rely on CI to catch things and run formatting occasionally.

Of course, you don’t get the same seamless experience as the VS Code / Cursor on-save, but the core functionality is fully available. The Biome project is exploring and improving editor integrations, so keep an eye out – there may be official support for more editors over time.

## Q: How do I know what rules Ultracite is enforcing? Is there a list?

Yes, check the [biome.json](https://github.com/haydenbleasel/ultracite/blob/main/biome.json) file in the GitHub repo for the rules that are enabled.
...
If you have a project where you temporarily don’t want Ultracite, you can simply not include the `extends: ["ultracite"]` in `biome.json` (or remove the config file). If the VS Code extension is installed, it may still try to format on save using Biome’s defaults.
