...
describe('editor rules configurations', () => {
  ...
  describe.each(editorConfigs)('$name configuration', ({ name, path, header, appendMode }) => {
    const editor = createEditorRules(name as keyof typeof EDITOR_RULES);
    const expectedContent = header ? `${header}\n\nmock rules content` : 'mock rules content';
    const expectedDir = path.includes('/') ? path.substring(0, path.lastIndexOf('/')) : '.';
    const cleanDir = expectedDir.startsWith('./') ? expectedDir.slice(2) : expectedDir;
    ...
    describe('create', () => {
      it(`should create ${path} with rules content`, async () => {
        ...
        if (cleanDir !== '.') {
          expect(mockMkdir).toHaveBeenCalledWith(cleanDir, { recursive: true });
        }
